...
bool MainGame::init()
{
	...
	label_score->setPosition(Vec2(label_score->getContentSize().width / 2 + 5,
		size.height - label_score->getContentSize().height / 2 - 5));
	...
	label_score_num_->setPosition(Vec2(label_score->getContentSize().width + 5, label_score->getPositionY()));
	...
	label_best_score->setPosition(Vec2(label_best_score->getContentSize().width / 2 + 5,
		label_score->getPositionY() - label_best_score->getContentSize().height - 5));
	...
	label_best_score_num_->setPosition(Vec2(label_best_score->getContentSize().width + 5, label_best_score->getPositionY()));
...
void MainGame::is_crash(float tm)
{
		...
		for (int i = bullet_list.size() - 1; i >= 0; --i)
		{
			...
			if (enemy_size.containsPoint(bullet->getPosition()))
			{
				...
				if (enemy->get_hp() <= 0)
				{
					...
					{
					case 100:
						AchievementLayer::showAchiecement(1);
						break;
					case 200:
						AchievementLayer::showAchiecement(2);
						break;
					case 500:
						AchievementLayer::showAchiecement(3);
						break;
					case 1000:
						AchievementLayer::showAchiecement(4);
						break;
					}
				}
			}
	...
	if (plane_support_ != NULL)
	{
		...
		{
			//添加支援物资效果子弹
			this->schedule(SEL_SCHEDULE(&MainGame::add_support_bullet), 0.1f, 50, 0);//设置调用50次
		    //停止普通子弹
			this->unschedule(SEL_SCHEDULE(&MainGame::add_bullet));
			//设置延迟执行添加普通子弹时间
			this->schedule(SEL_SCHEDULE(&MainGame::add_bullet), 0.1f, -1, 5.0f);//设置5s后延迟执行
...
void MainGame::hero_death()
{
	...
	int score_win = UserDefault::getInstance()->getIntegerForKey("best_score");
	if (score_ > score_win) {
		UserDefault::getInstance()->setFloatForKey("best_score", score_);
	}
	...
	Sound::getInstance()->play_effect(EFFECT_TYPE_DEATH);
...
void MainGame::suspend(Ref * ref)
{
	...
	//这里创建Layer 移植Android时无法电泳主线程界面中的下一帧
